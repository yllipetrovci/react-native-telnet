...
public class TelnetClientModule extends ReactContextBaseJavaModule {
  ...
  TelnetClientModule(ReactApplicationContext context) {
    ...
    @ReactMethod
    public void connect(Callback onSuccessCallback, Callback onErrorCallback) {
        telnetClientConfig = new TelnetClientConfig("209.73.216.51", 23,"gorjthatsmyrj","My3l@de.com");
        ...
        try {
            ...
            Thread reader = new Thread(new TelnetResponseMessageHandler(telnetClient,messageHolder,onSuccessCallback, onErrorCallback));
            reader.start();
   ...
   public  void sendCommand(String command, Callback onSuccessCallback, Callback onErrorCallback) {
    ...
    private void writeCommandToStream(OutputStream outstr, String command) throws IOException {
        byte[] weather2Bytes = (command + "\r\n").getBytes();
        int weather2BytesCounter = command.length() + 1;
        outstr.write(weather2Bytes, 0 , weather2BytesCounter);
        outstr.flush();
    }
    
    public  void addOptionalConnectionHandlers(TelnetClient telnetClient) throws InvalidTelnetOptionException, IOException {
